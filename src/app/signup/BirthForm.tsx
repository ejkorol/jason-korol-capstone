...
export default function BirthForm({ dobDate, dobTime, countries, states, cities, countryCode, stateCode, city, mbti, setDobDate, setDobTime, setCountryCode, setStateCode, setCity, setMbti }: BirthFormProps) {
  ...
  return (
    <>
      ...
      <Autocomplete size="lg" label="Select country" isRequired defaultItems={countries} selectedKey={countryCode ?? undefined} onSelectionChange={(key) => { setCountryCode(key) }}>
        {(country) => <AutocompleteItem key={country.value}>{country.displayValue}</AutocompleteItem>}
      </Autocomplete>
      <Autocomplete size="lg" label="Select state" isRequired defaultItems={states} selectedKey={stateCode ?? undefined} onSelectionChange={(key) => setStateCode(key)}>
        {(state) => <AutocompleteItem key={state.value}>{state.displayValue}</AutocompleteItem>}
      </Autocomplete>
      <Autocomplete size="lg" label="Select city" isRequired defaultItems={cities} selectedKey={city ?? undefined} onSelectionChange={setCity}>
